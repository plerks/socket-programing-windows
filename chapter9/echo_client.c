...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        when echoing Chinese character, the echo gets garbled. Need to run `chcp 936` in advance to set encoding to GBK and run with command
        line or compile with option `-fexec-charset=GBK`. I guess the typing tool gives process GBK encoded bytes since windows is GBK by default, so it's needed to set terminal encoding
        ...
        and git bash did't happen garbling without special operation. */
